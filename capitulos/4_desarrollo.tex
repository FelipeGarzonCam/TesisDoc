...
Aunque no se contempló dentro de los objetivos iniciales del proyecto, se llevó a cabo un despliegue operativo en un escenario comercial real: el establecimiento Gastro Uno 58 Bar. El propósito de esta fase adicional fue validar el comportamiento del software fuera del entorno de laboratorio y recolectar datos cualitativos sobre su nivel de adopción por parte de usuarios sin formación técnica previa.

La prueba piloto abarcó un periodo de un mes de operación continua. El despliegue comenzó con una jornada exclusiva de capacitación (día uno), durante la cual se instruyó al personal en el uso de las interfaces, el flujo de procesamiento de comandas y la autenticación de doble factor mediante tarjetas NFC. La plantilla del restaurante estuvo conformada por cuatro usuarios operativos: un administrador (quien asumió simultáneamente las funciones del módulo de Caja), dos meseras y una cocinera. Debido a que el establecimiento contaba con un servicio activo de internet, el sistema operó bajo las condiciones habituales de tráfico de una red comercial abierta, permitiendo contrastar su estabilidad frente a las pruebas de intranet aislada.

Para evaluar el impacto de la herramienta, la metodología definió la recolección de resultados a través de cuestionarios de satisfacción aplicados al personal al finalizar el mes de uso. Estos instrumentos buscan documentar la percepción cualitativa sobre la facilidad de uso, la agilidad en el servicio y la reducción de errores respecto al modelo de papel y lápiz previo.

Adicionalmente, esta implementación en producción se utilizó como modelo base para estructurar un análisis de viabilidad económica fundamentado en datos empíricos. A partir de este caso de estudio, se proyectó la generación de dos modelos de costos:
\begin{itemize}
	\item \textbf{Presupuesto de adaptación:} el cálculo de los equipos y componentes de hardware (lector RFID y microordenador) que fue estrictamente necesario proveer para poner en marcha el sistema en este restaurante en particular, aprovechando la infraestructura tecnológica que el local ya poseía.
	\item \textbf{Presupuesto de implementación inicial:} una proyección financiera integral calculada desde cero, simulando el escenario de un restaurante nuevo que requiere la adquisición total de los equipos informáticos y de red necesarios para cumplir con los requerimientos técnicos de la plataforma.
\end{itemize}